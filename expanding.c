...
/*   Updated: 2024/05/20 10:39:51 by sdiouane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// static char *exp(char *commande, s_env *export_i)
// {
//     char *exp_commande;
...
// 	{
// 		if (tmp->lst->cmd)
// 			tmp->lst->cmd = exp(tmp->lst->cmd,  export_i);
// 		else if (tmp->lst->redirection)
// 			tmp->lst->redirection = exp(tmp->lst->redirection, export_i);
...
noued_cmd *ft_expanding(ExecutionData **data, s_env *export_i) {
    ...
    // Parcourir la liste et effectuer l'expansion
    while (current) {
        if (current->cmd) {
            current->cmd = exp(current->cmd, export_i);
        }
        if (current->redirection) {
            current->redirection = exp(current->redirection, export_i);
        }
	...
	// {
	// 	if (data->lst->cmd)
	// 		data->lst->cmd = exp(data->lst->cmd,  export_i);
	// 	else if (data->lst->redirection)
	// 		data->lst->redirection = exp(data->lst->redirection, export_i);
	// 	printf("\nIN EXPANDING :\n");
